...
上表显示了Arduino 8位的支持的PWM模式。基本上，除了频率配置支持一切模式，PWM频率固定为32kHz。
...
Arduino SimpleFOClibrary已作为Arduino UNO库启动。经过优化，可用于这些类型的设备。但它并不局限于Arduino UNOs，你可以在Arduino MEGA和几乎所有带有ATMega328芯片或ATMega2560的板子上运行它。如Arduino Nano, Arduino pro-mini等。
...
对于初学者来说，一个有趣的示例板是基于ATMega328芯片的云台电机控制板。它集成了用于运行两个电机的无刷直流电机驱动硬件。虽然他们性能有限，但他们仍是伟大的初始设置。
...
Arduino <span>Simple<span>FOC</span>library</span>也将支持基于Arduino DUE board的ARM！

# Arduino IDE支持包

为了使用Arduino IDE中的Arduino DUE单板，请使用Arduino IDE单板管理器安装Arduino DUE单板支持包。